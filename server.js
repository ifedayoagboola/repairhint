...
//connect to mongoose
mongoose.connect("mongodb+srv://repairhint:12345@cluster0.muhuotz.mongodb.net/repairhint", {
...
app.post("/", async (req, res) => {
  ...
  if (url.length > 22) {
    ...
    User.register(
      { first: req.body.first, last: req.body.last, username: req.body.username, referrerSlug: referId },
      req.body.password,
      function (err, user) {
        if (err) {
    ...
    if (search) {
      ...
      User.register(
        { first: req.body.first, last: req.body.last, membername: req.body.membername, username: req.body.username },
        req.body.password,
        function (err, user) {
          if (err) {
            res.render("static/error/user-exist", { search: search });
